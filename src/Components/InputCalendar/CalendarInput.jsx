...
export const CalendarInput = ({ onDateChange }) => {
  ...
  const handleDateClick = (day) => {
    if (!day) return;
    const formattedDate = `${String(day).padStart(2, "0")}/${String(month + 1).padStart(2, "0")}/${year.toString().slice(-2)}`;
  ...
  return (
    <div className="relative" ref={calendarRef}>
      ...
      >
        ...
        <input 
          type="text" 
          value={selectedDate ? `${String(selectedDate.getDate()).padStart(2, "0")}/${String(selectedDate.getMonth() + 1).padStart(2, "0")}/${selectedDate.getFullYear().toString().slice(-2)}` : ''}
          readOnly 
      ...
      {showCalendar && (
        <div className="absolute bg-white rounded shadow-lg mt-2 z-50">
          <header className="flex items-center justify-between p-4">
            <p className="text-xl font-medium">{`${getMonthName(month)} ${year}`}</p>
            <div className="flex">
              <span 
